  ...
  return (
    <Card className="animate-in">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button variant="outline" onClick={onFavorite} className="shrink-0">{project.favorite ? '★' : '☆'}</Button>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <Badge>{project.category}</Badge><Badge>{project.status}</Badge><Badge>{project.priority}</Badge>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <Link to={`/projects/${project.id}/edit`} className="inline-flex items-center justify-center rounded-md border border-border bg-background px-3 py-2 text-sm font-medium hover:bg-muted">
          Edit
        </Link>
