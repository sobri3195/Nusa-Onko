...
export function HomePage({ projects }: { projects: Project[] }) {
  const byStatus = Object.groupBy(projects, (p) => p.status);
  const byCategory = Object.groupBy(projects, (p) => p.category);
  ...
  return <div className="space-y-4">
    ...
    <Card><h2 className="font-semibold mb-2">Ringkasan Status</h2>{Object.entries(byStatus).map(([k,v])=> <p key={k} className="text-sm">{k}: {v?.length ?? 0}</p>)}</Card>
    <Card><h2 className="font-semibold mb-2">Ringkasan Kategori</h2>{Object.entries(byCategory).map(([k,v])=> <p key={k} className="text-sm">{k}: {v?.length ?? 0}</p>)}</Card>
